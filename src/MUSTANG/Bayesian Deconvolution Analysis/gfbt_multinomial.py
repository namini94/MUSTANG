"""
Graph Fused Binomial Tree (GBFT) implementation

Acknowledgements:
GFBT implementation based on BayesTME implementation 
https://github.com/tansey-lab/bayestme/blob/main/src/bayestme/mcmc/gfbt_multinomial.py
"""
...
class GraphFusedBinomialTree:
    def __init__(self, n_classes, edges, trend_order=0, lam2=1e-5, pg_seed=42, stability=1e-6,
        ...
        #neighbors_tot = pandas.read_csv('/scratch/user/naminiyakan/Spatial/Data/Maynard/Total/neighbors_tot.csv',header=0,index_col=0) #For Maynard
        #neighbors_tot = pandas.read_csv('/scratch/user/naminiyakan/Spatial/Data/Total/neighbors_tot.csv',header=0,index_col=0) #For bone_morrow_data
        neighbors_tot = pandas.read_csv('/scratch/user/naminiyakan/Spatial/Data/MouseBrain/Total/neighbors_tot.csv',header=0,index_col=0) #For Mouse brain sagittal 
        neighbors_tot.reset_index(drop=True, inplace=True)
        neigh = neighbors_tot.to_numpy()
        self.D = utils.construct_edge_adjacency(neighbors=neigh)
        #####
